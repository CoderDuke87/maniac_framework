...
Class ManiacSlider
	...
	Field bWithEditField:Bool = True 
	
	Field tfValue:ManiacTextfield
	Public 
	#Rem monkeydoc
	...
	Method New(_X:Float,_Y:Float,_Width:Float,_Height:Float,_ValueFrom:Float = 0,_ValueTo:Float = 100)
		...
		tfValue = New ManiacTextfield(_X+_Width+5,_Y,_Width*0.2,_Height,"",0)
	End Method 
	...
	Method setValue:Void(_Value:Float)
		Local p:Float = (_Value - ValueFrom)/ (ValueTo - ValueFrom)
		SliderX = p * Width + X
	End Method 
	#Rem monkeydoc
	...
	Method Update()
		...
		If bWithEditField= True
			...
			If KeyHit(KEY_ENTER) And tfValue.bEditing = True
				setValue(Int(tfValue.getText()))
			Endif 
		Endif
	End Method 
